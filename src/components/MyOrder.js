...
export default function MyOrder() {
  ...
  const localStorageKey = (key) => localStorage.getItem(key);

  useEffect(() => {
    const authToken = localStorageKey("authToken");
    const userEmail = localStorageKey("userEmail");

    if (authToken && userEmail) {
      fetchMyOrder(userEmail);
    } else {
      setOrderData([]);
    }
  }, [localStorageKey("authToken")]);
  const fetchMyOrder = async (userEmail) => {
    try {
      ...
      if (response.ok) {
        ...
        setOrderData(Array.isArray(orderDataArray) ? orderDataArray : []);
      } else {
